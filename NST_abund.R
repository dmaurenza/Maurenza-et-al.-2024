...
my_fun <- function(cat, matrix){
  ...
  list_cat_500 <- data.frame(Low.RC.bray = as.double(), Inter.RC.bray = as.double(), High.RC.bray = as.double(), study_id = as.character(), stringsAsFactors=FALSE)
  
  list_cat_1k <- data.frame(Low.RC.bray = as.double(), Inter.RC.bray = as.double(), High.RC.bray = as.double(), study_id = as.character(), stringsAsFactors=FALSE)
  
  list_cat_2k <- data.frame(Low.RC.bray = as.double(), Inter.RC.bray = as.double(), High.RC.bray = as.double(), study_id = as.character(), stringsAsFactors=FALSE)
  
  list_cat_4k <- data.frame(Low.RC.bray = as.double(), Inter.RC.bray = as.double(), High.RC.bray = as.double(), study_id = as.character(), stringsAsFactors=FALSE)
  
  list_cat_8k <- data.frame(Low.RC.bray = as.double(), Inter.RC.bray = as.double(), High.RC.bray = as.double(), study_id = as.character(), stringsAsFactors=FALSE)
  ...
  for (i in 1:length(matrix)){ # length(matrix)
    ...
    for(o in 1:5){
      ...
      FC <- as.data.frame(matrix.o[cat_vector[o]])
      ...
      n_sites <- matrix.o %>% 
        count(matrix.o[cat_vector[o]]) %>% 
        arrange(desc(1)) %>% 
        filter(n>2) %>% 
        pull(1)
      matrix.sites <- matrix.o %>% 
        filter(.[[cat_vector[o]]] %in% n_sites)
      ...
      # Calculanting individuals of species frequencies
      ...
      meta.com=t(as.matrix(colMeans(freq.grps)))
      ...
      RC.bray <- tnst[[1]][,c("name1", "name2", "RC.bray")] %>% 
        mutate(RC.bray = as.numeric(RC.bray))
      ...
      low.RC <- mean(groups[groups$group=="Low", "RC.bray"])
      inter.RC <- mean(groups[groups$group=="Intermediate", "RC.bray"])
      high.RC <- mean(groups[groups$group=="High", "RC.bray"])
      
      
      result <- data.frame(Low.RC.bray = as.double(), Inter.RC.bray = as.double(), High.RC.bray = as.double(), study_id = as.character(), stringsAsFactors=FALSE)
...
NST_abund <- my_fun(cat = cat_vector, matrix = matrix_cat)

saveRDS(NST_abund[[1]], "NST/NST_abund_500.Rdata")
saveRDS(NST_abund[[2]], "NST/NST_abund_1k.Rdata")
saveRDS(NST_abund[[3]], "NST/NST_abund_2k.Rdata")
saveRDS(NST_abund[[4]], "NST/NST_abund_4k.Rdata")
saveRDS(NST_abund[[5]], "NST/NST_abund_8k.Rdata")
